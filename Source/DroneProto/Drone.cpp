...
void ADrone::ApplyDodgeVisualDirectionLocally(const FVector& DodgeVisualDirection)
{
	...
	const FVector DirectionWS = DodgeVisualDirection.GetSafeNormal2D();
	if (DirectionWS.IsNearlyZero())
	{
		return;
	}
	...
	const FVector DirectionLS = DodgeTeleportVFX->GetComponentTransform()
		...
		.GetSafeNormal();
	DodgeTeleportVFX->SetVariableVec3(FName(TEXT("User.DodgeDirectionLS")), DirectionLS);
	DodgeTeleportVFX->SetVariableVec3(FName(TEXT("User.DodgeDirectionWS")), DirectionWS);
}
